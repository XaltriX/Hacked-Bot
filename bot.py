...
from thackedbot import CUSTOM_REPLY
...
def save_remaining_tokens(tokens):
    """Save remaining tokens that couldn't be deployed"""
    try:
        if tokens:
...
def get_vps_capacity():
    """Calculate VPS capacity based on resources"""
    try:
...
async def startup_bots(tokens, dashboard_bot=None, notify_chat_id=None):
    """Start bots in batches with proper error handling and limit checking"""
    global MAX_BOTS_LIMIT
    ...
    try:
        ...
        async def cmd_capacity(msg: types.Message):
            try:
                ...
                logger.error(f"Error in capacity command: {e}")
        ...
        async def handle_document(msg: types.Message):
            try:
                ...
                elif msg.text and re.match(r'^\d{6,10}:[A-Za-z0-9_-]{20,}'
                    , msg.text.strip()):
